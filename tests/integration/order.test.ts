...
describe("POST /order", () => {
    ...
    it("should respond with status 401 if there is no session for given token", async () => {
        ...
        const response = await server.post("/order").set("Authorization", `Bearer ${token}`)
    ...
    describe("when token is valid", () => {
        ...
        it("should respond with 400 if body is invalid", async () =>{
            ...
            const response = await server.post("/order").set("Authorization", `Bearer ${token}`).send(invalidBody);
